...
for num in range (10):
    filenames = os.listdir('resultsDenoisingSparse')    
    ...
    for item in filenames:
        ...
        if (url != target_filepath):
            dataset = pandas.read_csv('resultsDenoisingSparse/'+url, header = None, engine='python')
    ...
    target_number = pandas.read_csv('resultsDenoisingSparse/'+target_filepath, header = None)
    ...
    with open('svmOneClassWithDenoisingAE/resultSVMauthDenoisingSparse' + str(num) +'.txt', 'w') as f:
    ...
    with open('svmOneClassWithDenoisingAE/resultSVMauthAccuracy.txt', 'a') as f:
       f.write("User: " + str(num) + "\nFAR: " + str("%.5f" % FAR) + "\nFRR: " + str("%.5f" % FRR) + "\n\n\n")
      
with open('svmOneClassWithDenoisingAE/resultSVMauthAccuracy.txt', "a") as myfile:
