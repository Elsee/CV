...
def ae(input_shape, X, x_train_noisy, X_test, x_test_noisy):
    ...
    encoding_dim = math.floor(input_shape/3)
    # this is our input placeholder
    input_img = Input(shape=(input_shape,))
    # "encoded" is the encoded representation of the input
    encoded = Dense(input_shape, activation='relu')(input_img)
    encoded = Dense(math.floor(input_shape/2), activation='relu')(input_img)
    encoded = Dense(encoding_dim, activation='linear',
                    activity_regularizer=regularizers.l2(0.00001))(encoded)
    # "decoded" is the lossy reconstruction of the input
    decoded = Dense(math.floor(input_shape/2), activation='relu')(encoded)
